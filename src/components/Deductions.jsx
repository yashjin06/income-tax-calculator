...
const Deductions = ({ data, updateData }) => {
  ...
  useEffect(() => {
    ...
    const c_ccc_ccd1 = (parseFloat(ded.sec80c) || 0) + (parseFloat(ded.sec80ccc) || 0) + (parseFloat(ded.sec80ccd1) || 0)
    ...
    const others = (parseFloat(ded.sec80d) || 0) + 
  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <h2 className="text-xl font-bold">Chapter VI-A Deductions</h2>
        <div className="bg-primary-light" style={{ padding: '0.5rem 1rem', borderRadius: 'var(--radius-md)', background: 'rgba(79, 70, 229, 0.1)', color: 'var(--primary)', fontWeight: 'bold' }}>
          Total Estimated Deductions: ₹ {totalDeductions.toLocaleString('en-IN')}
        </div>
      </div>
      ...
      <div className="card p-6 slide-up mb-6">
        <h3 className="text-lg font-bold mb-4">Investments (80C / 80CCC / 80CCD)</h3>
        <p style={{fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '1rem'}}>Combined maximum limit for 80C, 80CCC, and 80CCD(1) is ₹ 1,50,000.</p>
      ...
      <div className="card p-6 slide-up" style={{ animationDelay: '0.1s' }}>
        <h3 className="text-lg font-bold mb-4">Other Deductions</h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
