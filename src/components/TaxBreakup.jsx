...
const TaxBreakup = ({ data, textStyle = "professional" }) => {
  const isGenZ = textStyle === "genz";
  const results = useMemo(() => computeTax(data), [data]);

  const labels = {
    title: isGenZ ? "Tax Tea ☕ (The Receipts)" : "Tax Calculation Breakup",
    subtitle: isGenZ
      ? "A detailed breakdown of how much the govt is taking from your bags."
      : "Detailed slab-wise breakdown of your computed Tax Liability.",
    normalTableTitle: isGenZ ? "Govt's Cut (Slab-wise Math)" : "Slab-wise Normal Tax Computation",
    tableHeaderSlab: isGenZ ? "Profit Tier" : "Income Slab",
    tableHeaderRate: isGenZ ? "Govt's Tax Rate" : "Tax Rate",
    tableHeaderIncome: isGenZ ? "Money in Tier (₹)" : "Income in Slab (₹)",
    tableHeaderTax: isGenZ ? "Tax on this Tier (₹)" : "Tax Amount (₹)",
    specialTitle: isGenZ ? "Special Taxes & Tweaks" : "Special Taxes & Adjustments",
    specialBreakdownHeader: isGenZ
      ? "Tax on Side Hustles Breakdown"
      : "Tax on Special Incomes Breakdown",
    totalNormalTaxLabel: isGenZ ? "TOTAL RAW TAX" : "TOTAL NORMAL TAX",
    totalSpecialTaxLabel: isGenZ ? "Total Hustle Tax" : "Total Special Tax",
    rebateLabel: isGenZ ? "Less: Rebate (Govt Mercy)" : "Less: Rebate u/s 87A",
    ...
    noIncome: isGenZ
      ? "No taxable normal income found. You're chillin'."
      : "No taxable normal income to compute.",
  };
  ...
  return (
    <div className="fade-in">
      ...
      <div className="card p-6 slide-up" style={{ animationDelay: "0.1s" }}>
        <h2 className="text-xl font-bold mb-6">{labels.normalTableTitle}</h2>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "600px" }}>
            <thead>
              ...
              >
                <th style={{ padding: "1rem", textAlign: "left", color: "var(--text-muted)" }}>
                  {labels.tableHeaderSlab}
                </th>
                <th style={{ padding: "1rem", textAlign: "center", color: "var(--text-muted)" }}>
                  {labels.tableHeaderRate}
                </th>
                <th style={{ padding: "1rem", textAlign: "right", color: "var(--text-muted)" }}>
                  {labels.tableHeaderIncome}
                </th>
                <th style={{ padding: "1rem", textAlign: "right", color: "var(--text-muted)" }}>
                  {labels.tableHeaderTax}
                </th>
              </tr>
            </thead>
            <tbody>
              {results.taxBreakup && results.taxBreakup.length > 0 ? (
                results.taxBreakup.map((row, idx) => (
                  <tr key={idx} style={{ borderBottom: "1px solid var(--input-border)" }}>
                    ...
                    <td style={{ padding: "1rem", textAlign: "right", fontSize: "1.05rem" }}>
                    ...
                    <td
                      style={{
                        ...
                        textAlign: "right",
                        fontSize: "1.05rem",
                        color: row.tax > 0 ? "var(--danger)" : "var(--text-main)",
                      }}
                    >
            ...
            <tfoot>
              ...
              >
                ...
                >
                  {labels.totalNormalTaxLabel}
                </td>
                <td style={{ padding: "1.25rem 1rem", textAlign: "right", fontSize: "1.15rem" }}>
                  {results.taxableNormalIncome > 0
                    ? results.taxableNormalIncome.toLocaleString("en-IN")
                    : "0"}
                </td>
                <td
                  style={{
                ...
                >
                  {results.normalTax.toLocaleString("en-IN")}
                </td>
              </tr>
            </tfoot>
          </table>
      ...
      <div className="card p-6 mt-6 slide-up" style={{ animationDelay: "0.2s" }}>
        <h2 className="text-xl font-bold mb-4">{labels.specialTitle}</h2>
        <div style={{ display: "grid", gap: "1rem" }}>
          {results.specialTaxBreakup && results.specialTaxBreakup.length > 0 ? (
            ...
            >
              ...
              {results.specialTaxBreakup.map((item, idx) => (
                <div
                  ...
                  style={{
                    ...
                    borderBottom:
                      idx < results.specialTaxBreakup.length - 1
              ...
              >
                <span style={{ fontWeight: 600, color: "var(--primary)" }}>
                  {labels.totalSpecialTaxLabel}
                </span>
                <span
                  style={{ fontSize: "1.1rem", color: "var(--primary)", fontWeight: "bold" }}
                >
                  + ₹ {results.specialTax.toLocaleString("en-IN")}
                </span>
              </div>
            </div>
          ) : (
            ...
            >
              <span style={{ fontWeight: 500 }}>{labels.totalSpecialTaxLabel}</span>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", padding: "1rem" }}>
              <span style={{ fontWeight: 500 }}>{labels.rebateLabel}</span>
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", padding: "1rem" }}>
              <span style={{ fontWeight: 500 }}>{labels.surchargeLabel}</span>
              <span style={{ fontSize: "1.1rem", color: "var(--danger)" }}>
            ...
            {results.surcharge > 0 && results.surchargeBreakup && (
              ...
              >
                {results.surchargeBreakup.amountNormal > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.25rem" }}>
                    <span>On Normal Tax @ {(results.surchargeBreakup.rateNormal * 100).toFixed(0)}%</span>
                    <span>₹ {Math.round(results.surchargeBreakup.amountNormal).toLocaleString("en-IN")}</span>
                  </div>
                )}
                {results.surchargeBreakup.amountSpecial > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "0.25rem" }}>
                    <span>On Special Tax @ {(results.surchargeBreakup.rateSpecial * 100).toFixed(0)}%</span>
                    <span>₹ {Math.round(results.surchargeBreakup.amountSpecial).toLocaleString("en-IN")}</span>
                  </div>
                )}
                {results.surchargeBreakup.marginalRelief > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", color: "var(--success)" }}>
                    <span>Less: Marginal Relief</span>
                    <span>(₹ {Math.round(results.surchargeBreakup.marginalRelief).toLocaleString("en-IN")})</span>
                  </div>
                )}
              </div>
            )}
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", padding: "1rem" }}>
              <span style={{ fontWeight: 500 }}>{labels.cessLabel}</span>
              <span style={{ fontSize: "1.1rem", color: "var(--danger)" }}>
            ...
            {results.cess > 0 && (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <span>
                    Calculated on (Tax + Surcharge) value of ₹{" "}
                    {Math.round(results.taxPlusSurcharge).toLocaleString("en-IN")}
                  </span>
                </div>
              </div>
            )}
              ...
              {results.tdsPaid > 0 && (
                ...
                >
                  ...
                  <span style={{ color: "var(--success)" }}>
                    - ₹ {results.tdsPaid.toLocaleString("en-IN")}
                  </span>
                </div>
              )}
              {results.advanceTaxPaid > 0 && (
                ...
                >
                  ...
                  <span style={{ color: "var(--success)" }}>
                    - ₹ {results.advanceTaxPaid.toLocaleString("en-IN")}
                  </span>
                </div>
              )}
              {results.selfAssessmentTaxPaid > 0 && (
                ...
                >
                  ...
                  <span style={{ color: "var(--success)" }}>
                    - ₹ {results.selfAssessmentTaxPaid.toLocaleString("en-IN")}
                  </span>
                </div>
              )}
            </div>
          ...
          <div
            style={{
              ...
              padding: "1.25rem",
              background:
                results.netTaxPayable < 0 ? "rgba(16, 185, 129, 0.1)" : "var(--glass-bg)",
              color: results.netTaxPayable < 0 ? "var(--success)" : "var(--primary)",
              borderRadius: "var(--radius-lg)",
              marginTop: "0.5rem",
              fontWeight: "bold",
              borderTop: `2px solid ${
                results.netTaxPayable < 0 ? "var(--success)" : "var(--primary)"
              }`,
            }}
          >
            <span style={{ fontWeight: "bold", fontSize: "1.25rem" }}>
              {results.netTaxPayable < 0 ? labels.refundLabel : labels.finalPayableLabel}
            </span>
            <span style={{ fontWeight: "bold", fontSize: "1.25rem" }}>
              ₹ {Math.abs(results.netTaxPayable).toLocaleString("en-IN")}
            </span>
          </div>
        </div>
      </div>
